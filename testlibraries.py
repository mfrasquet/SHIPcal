...
def print_results(integration,fluid,plotVars): #Function to compare the results
    ...
    #Y se escriben las los numeros de la listas en el archivo
    f.write("{}    {}    {}    {}    {}\n".format(integration, fluid, round(plotVars['Production_max'],2), round(plotVars['Production_lim'],2), round(sum(plotVars['Q_charg']),2)))
    ...
    return integration, fluid, round(plotVars['Production_max'],2), round(plotVars['Production_lim'],2), round(sum(plotVars['Q_charg']),2)
...
#Abrir archivo para modificarlo
f=open("master","a+")
#Se escriben los encabezads de las listas en el archivo ya en el formato para importar a caja
f.write('Integración    Fluido    Prod_max    Prod_lim    sum(Qcharg)   \n')
f.close()
...
for integration in ['SL_L_P', 'SL_L_PS', 'SL_L_RF', 'SL_L_S', 'SL_L_S_PH']:#Tests for liquid phase integrations #Checked 'SL_L_P', 'SL_L_PS', 'SL_L_RF', 
    ...
    print_results(integration,inputsDjango['fluid'],plotVars)
