...
def operationSimple2(itercontrol,fluidInput,bypass,T_in_flag,T_in_K_old,T_in_C_AR,T_out_K_old,T_in_C,P_op_Mpa,bypass_old,T_out_C,temp,theta_i_rad,DNI,IAM,Area,n_coll_loop,num_loops,mofProd,coef_flow_rec,m_dot_min_kgs,Q_prod_rec_old,sender,coll_par):
...
#SL_L_P Supply level with liquid heat transfer media Parallel integration pg52 
    ...
    if flow_rate_kgs<=m_dot_min_kgs and T_out_K>T_in_K: #El caudal necesario para obtener la temp de salida es inferior al mínimo
        ...
        bypass.append("REC")
        newBypass="REC"
        Perd_termicas = Perd_termicas*num_loops/(1000*factor) #Takes into account all the loops, before it was only the losses of one serie (one loop)
...
def operationDSG2(itercontrol,bypass,bypass_old,T_out_K_old,T_in_C,P_op_Mpa,temp,REC_type,theta_i_rad,DNI,Long,IAM,Area,n_coll_loop,rho_optic_0,num_loops,mofProd,coef_flow_rec,m_dot_min_kgs,x_desing,Q_prod_rec_old,subcooling):
#SL_L_P Supply level with liquid heat transfer media Parallel integration pg52 
    Perd_termicas=0
...
def operationDSG_Rec2(itercontrol,m_dot_min_kgs,bypass,SD_min_energy,T_SD_K_old,SD_mass,SD_energy_old,T_in_C,P_op_Mpa,temp,REC_type,theta_i_rad,DNI,Long,IAM,Area,n_coll_loop,rho_optic_0,num_loops,mofProd,x_desing,PerdSD):
#SL_L_P Supply level with liquid heat transfer media Parallel integration pg52 
    Perd_termicas=0
    if itercontrol=='paso_10min':
        factor=6
