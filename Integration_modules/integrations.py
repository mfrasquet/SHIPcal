...
def operationOnlyStorageSimple(
    fluidInput, T_max_storage, T_in_K_old, P_op_Mpa, temp, theta_i_rad, DNI, IAM, Area, n_coll_loop, num_loops, mofProd, flow_rate_kgs, sender, coll_par
):
...
def directopearationSimple(
    fluidInput, T_out_C, T_in_C, P_op_Mpa, temp, DNI, IAM, Area, n_coll_loop, num_loops, theta_i_rad, bypass, T_in_flag, T_in_C_AR, design_flow_rate, mofProd, sender, coll_par
):
    if sender == "CIMAV":
        ...
        if T_in_flag == 1 and fluidInput == "water":
            ...
            T_in_K = T_in_C + 273
        T_out_K, Perd_termicas = IT_temp(
            fluidInput, T_in_K, P_op_Mpa, temp, theta_i_rad, DNI, IAM, Area, n_coll_loop, design_flow_rate, **coll_par
        )  # Here the T_out_K is the desired outlet temperature, it is used to calculate an average CP and the coll parameters
    ...
    Q_prod *= num_loops * mofProd
    Perd_termicas = (
        Perd_termicas * num_loops + (1 - mofProd) * Q_prod
    )  # Takes into account all the loops, before it was only the losses of one serie (one loop) and the losses by the modificator mofProd
    bypass.append("ON")
...
def operationSimple(
    ...
    if flow_rate_kgs <= m_dot_min_kgs and T_out_K > T_in_K:  # El caudal necesario para obtener la temp de salida es inferior al mínimo
        ...
        if sender == "CIMAV":
            ...
            T_out_K, Perd_termicas = IT_temp(
                fluidInput, T_in_K, P_op_Mpa, temp, theta_i_rad, DNI, IAM, Area, n_coll_loop, flow_rate_rec, **coll_par
            )  # Here the T_out_K is the desired outlet temperature, it is used to calculate an average CP and the coll parameters
...
def outputOnlyStorageSimple(
    ...
    if T_min_storage >= T_alm_K_old:  # The storage is still under the minimum temperatura -> Charge
        ...
        if ((Q_prod) + energy_stored) < (
            storage_max_energy - storage_energy_old
        ):  # A.1 still room in the storage. Is the total available energy less than the posible available energy? if so all of it is stored.
            ...
            storage_energy_new = (
                storage_energy_old + Q_prod
            ) * 3600  # KJ #The new energy that the storage will have is the previous total energy plus the produced. Nothing is send to the process because the energy is used to heat the storage
            ...
            T_alm_new = storage_energy_new / (
                storage_Cp * almVolumen * (0.001) * (storage_rho)
            )  # in K #The new temperature of the storage with the energy that gained in this hour.
            ...
            if (
                fluidInput == "water" and IAPWS97(P=P_op_Mpa, T=T_alm_new).x > 0
            ):  # Steam in the storage danger!! #If the fluid is water, it is possible that the temperature is so high that it became steam
        ...
        if Q_prod + energy_stored <= Demand:  # B.2 Complete discharge #All the available energy is less than the demand so all has to be used.
            ...
            storage_energy_new = (
                storage_energy_old - Q_discharg
            ) * 3600  # From the total energy that the storage had in the previous step, now it has Q_discharg less energy (which should be the same as the same as the minimum possible energy that the storage could have in total)
            ...
            energy_stored = (
                energy_stored - Q_discharg
            )  # (Demand-Q_prod) #New state of the storage #The new available energy is the previous available energy minus the energy necessary to complete the demand
            Q_charg = 0  # No charge
            ...
            Q_defocus = 0  # Nothing is wasted
            storage_energy_new = (
                storage_energy_old + Q_prod - Demand
            ) * 3600  # KJ #The new energy available is the total previous energy plus the produced energy minus the demanded energy
            ...
            if ((Q_prod - Demand) + energy_stored) < energStorageUseful and (T_alm_K_old < T_max_storage):  # B.3.2 Still room in the storage for the full production
                ...
                if (
                    fluidInput == "water" and IAPWS97(P=P_op_Mpa, T=T_alm_new).x > 0
                ):  # Steam in the storage danger!! #In the unlikely case that the new temperature is over the vapor temperatue of water assign a new storage temperature, some energy is lost and should be calculated.
                ...
                if T_alm_K_old < T_max_storage:  # In the previous hour the storage temperature was less than the maximum posible storage temperature
                    ...
                    Q_useful = (
                        Demand + Q_charg
                    )  # (energStorageUseful-energy_stored) #The energy that could be used is the one to cover the deman plus the one to fully charge the storage
                ...
                Q_defocus = (
                    Q_prod - Q_useful
                )  # Demand-Q_charg #Some energy is lost, the lost energy is the difference between what was used (charge up the storage and to cover the demand) and the one produced
...
def PDCLatent(d_int, P_in, x_in, Long, m_dot, granoEpsilon):
    ...
    # Friedel formula
    R = A + 3.43 * x_in**0.685 * (1 - x_in) ** 0.24 * (density_liq / density_steam) ** 0.8 * (viscosity_steam / viscosity_liq) ** 0.22 * (
        1 - (viscosity_steam / viscosity_liq)
    ) ** 0.89 * Fr_liq ** (-0.047) * We_liq ** (-0.0334)
