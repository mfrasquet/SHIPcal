...
def SolarEQ_simple(
    Month, Day, Hour, Lat, Huso
):  # Returns the hour angle (W) [rad], sun elevation angle[rad], azimuth angle[rad], declination [rad] and zenithal angle [rad] of the sun for each the specified hour, latitude[°], anf time zone given in the inputs.
...
def theta_IAMs(SUN_AZ_DP, SUN_ELV_DP, beta, orient_az_rad):
    ...
    # Caso general
    theta_transv_rad = np.arctan(
        (np.cos(SUN_ELV_DP) * np.sin(azimuth_solar_corr_rad - orient_az_rad))
        / (np.sin(SUN_ELV_DP - beta) + np.sin(beta) * np.cos(SUN_ELV_DP) * (1 - np.cos(azimuth_solar_corr_rad - orient_az_rad)))
    )
...
def theta_IAMs_v2(SUN_AZ, SUN_ELV, LONG_INCL, HEAD, ROLL):
    ...
    sunXl = (
        sunX * (np.cos(alpha) * np.cos(beta) + np.sin(alpha) * np.sin(beta) * np.sin(theta))
        - sunY * (np.cos(alpha) * np.sin(beta) - np.cos(beta) * np.sin(alpha) * np.sin(theta))
        + sunZ * np.sin(alpha) * np.cos(theta)
    )
    sunYl = sunY * np.cos(beta) * np.cos(theta) - sunZ * np.sin(theta) + sunX * np.sin(beta) * np.cos(theta)
    sunZl = (
        sunY * (np.sin(alpha) * np.sin(beta) + np.cos(alpha) * np.cos(beta) * np.sin(theta))
        - sunX * (np.cos(beta) * np.sin(alpha) - np.cos(alpha) * np.sin(beta) * np.sin(theta))
        + sunZ * np.cos(alpha) * np.cos(theta)
    )
...
def theta_IAMs_v3(SUN_AZ, SUN_ELV, LONG_INCL, HEAD, ROLL):  # SUN_AZ [rad],SUN_ELV[rad],LONG_INCL[°],HEAD[°],ROLL[°] because those were the input units in the original version
    ...
    transversal_axe = np.cross(
        A, longitudinal_axe
    )  # Area perpendicular to the surface, transveral and longitudinal axe are the perpendicular axes on the collector surface they may or may not coincide with the X and Y axes
...
def rotation_matrix(u, angle):  # angle in radians, vector u normalized
    ...
    R = np.array(
        [
            [cang + (1 - cang) * ux**2, ux * uy * (1 - cang) - uz * sang, ux * uz * (1 - cang) + uy * sang],
...
def Meteo_data(
    file_meteo, sender="notCIMAV", optic_type="0"
):  # This function exports the TMY file to 'data', the DNI and temperature array of values for  each hour in the year from the file_meteo path to file
...
def SolarData(file_loc, mes_ini_sim, dia_ini_sim, hora_ini_sim, mes_fin_sim, dia_fin_sim, hora_fin_sim, sender="notCIMAV", Lat=0, Huso=0, optic_type="0"):
    ...
    hour_year_ini = calc_hour_year(
        mes_ini_sim, dia_ini_sim, hora_ini_sim
    )  # Calls a function within this same script yo calculate the corresponding hout in the year for the day/month/hour of start and end
    ...
    for step in range(0, sim_steps):
        # Posicion solar
        W, SUN_ELV, SUN_AZ, DECL, SUN_ZEN = SolarEQ_simple(
            month_sim[step], day_sim[step], hour_sim[step], Lat, Huso
        )  # calls another function in within this script that calculates the solar positional angles for the specfied hour of the day and month
        W_sim[step] = W
    ...
    output = np.column_stack(
        (month_sim, day_sim, hour_sim, hour_year_sim, W_sim, SUN_ELV_sim, SUN_AZ_sim, DECL_sim, SUN_ZEN_sim, DNI_sim, temp_sim, step_sim)
    )  # Arranges the calculated data in a massive array with the previusly calculated vector as columns
